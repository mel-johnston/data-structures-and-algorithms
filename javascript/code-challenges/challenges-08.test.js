...
const sortByChildren = (charArray) => {
  const alphArray = charArray.sort((a, b) => a.name > b.name ? 1 : -1);
  return alphArray.sort((a, b) => a.children.length - b.children.length);
};
...
const isCapitalized = (str) => {
  let regex = /[A-Z]\w+/g;
  let capArray = str.match(regex);
  return capArray ? capArray : [];
};
