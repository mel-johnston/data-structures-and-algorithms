...
const screenForNames = (arr) => {
  const regex = new RegExp(/^(Mr\.|Mrs\.|Ms\.|Dr\.) ?[A-Za-z]+/);
...
const toTitleCase = (arr) => arr.map(str => str.replace(str[0], str[0].toUpperCase()));
...
const sortBy = (property, arr) => {
  if (typeof (arr[0][property]) === 'string') {
    let newArr = arr.sort((a, b) => a[property] > b[property] ? 1 : -1);
    return newArr;
  } else {
    let newArr = arr.sort((a, b) => a[property] - b[property]);
    return newArr;
  }
};
